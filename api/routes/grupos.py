...
from api.services import grupos as grupo_service
...
async def listar_grupos(
    ...
    return grupo_service.list(empresa_id=empresa_id, ativo=ativo)


@router.get("/{grupo_id}", response_model=GrupoResponse)
async def buscar_grupo(grupo_id: int):
    grupo = grupo_service.get_by_id(grupo_id)
    if not grupo:
...
async def criar_grupo(grupo: GrupoCreate):
    return grupo_service.create(grupo.model_dump())


@router.patch("/{grupo_id}", response_model=GrupoResponse)
async def atualizar_grupo(grupo_id: int, grupo: GrupoUpdate):
    grupo_atualizado = grupo_service.update(grupo_id, grupo.model_dump(exclude_unset=True))
...
async def deletar_grupo(grupo_id: int):
    sucesso = grupo_service.delete(grupo_id)
    if not sucesso:
        raise HTTPException(status_code=404, detail="Grupo não encontrado")
