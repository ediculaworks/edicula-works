...
from api.services import sprints as sprint_service
...
async def listar_sprints(
    ...
    return sprint_service.list(empresa_id=empresa_id, projeto_id=projeto_id, status=status)


@router.get("/ativa", response_model=SprintResponse)
async def buscar_sprint_ativa(empresa_id: int = Query(1)):
    sprint = sprint_service.get_active(empresa_id=empresa_id)
    if not sprint:
...
async def buscar_sprint(sprint_id: int):
    sprint = sprint_service.get_by_id(sprint_id)
    if not sprint:
...
async def criar_sprint(sprint: SprintCreate):
    return sprint_service.create(sprint.model_dump())


@router.patch("/{sprint_id}", response_model=SprintResponse)
async def atualizar_sprint(sprint_id: int, sprint: SprintUpdate):
    sprint_atualizado = sprint_service.update(sprint_id, sprint.model_dump(exclude_unset=True))
...
async def deletar_sprint(sprint_id: int):
    sucesso = sprint_service.delete(sprint_id)
    if not sucesso:
        raise HTTPException(status_code=404, detail="Sprint não encontrada")
...
async def iniciar_sprint(sprint_id: int):
    sprint = sprint_service.update(sprint_id, {"status": "ativa"})
    if not sprint:
        raise HTTPException(status_code=404, detail="Sprint não encontrada")
    return sprint
...
async def concluir_sprint(sprint_id: int):
    ...
    sprint = sprint_service.update(sprint_id, {"status": "concluida", "data_conclusao": date.today()})
    if not sprint:
        raise HTTPException(status_code=404, detail="Sprint não encontrada")
    sprint_service.update_pontos(sprint_id)
    return sprint
